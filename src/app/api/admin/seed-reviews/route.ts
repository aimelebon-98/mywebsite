...
interface ProductSeed {
  reviews: ReviewSeed[];
  // Inflated public reviewCount (shows social proof from many buyers,
  // while only displaying the top curated ones as text reviews)
  displayReviewCount: number;
}

const PRODUCT_REVIEWS: Record<string, ProductSeed> = {
  // ==================== NIKE AIR FORCE 1 (most popular - 47 reviews) ====================
  "nike-air-force-1-cream-grey-rope-laces": {
    displayReviewCount: 47,
    reviews: [
      ...
      { customerName: "Ibrahim Toure", rating: 5, comment: "Best purchase of the year! The Air Force 1s are a classic and this version with rope laces is unique. Fast delivery.", commentFr: "Meilleur achat de l'ann\u00e9e! Les Air Force 1 sont un classique et cette version avec les lacets en corde est unique. Livraison rapide.", verified: true, daysAgo: 160, avatarIdx: 9 },
    ],
  },

  // ==================== ALEXANDER MCQUEEN (very popular - 31 reviews) ====================
  "alexander-mcqueen-tread-slick-black-white": {
    displayReviewCount: 31,
    reviews: [
      ...
      { customerName: "Claire Petit", rating: 5, comment: "Finally! I found the Alexander McQueen I was looking for. The quality is impeccable and the style timeless. Thanks!", commentFr: "Enfin! J'ai trouv\u00e9 les Alexander McQueen que je cherchais. La qualit\u00e9 est irr\u00e9prochable et le style intemporel. Merci!", verified: true, daysAgo: 94, avatarIdx: 8 },
  ...
  // ==================== OFF-WHITE BOOT (popular - 23 reviews) ====================
  "off-white-chunky-trail-boot-tan": {
    displayReviewCount: 23,
    reviews: [
      ...
      { customerName: "Sophie Laurent", rating: 5, comment: "Superb quality! The Off-White boots are an investment but these are incredible. The leather is premium.", commentFr: "Superbe qualit\u00e9! Les bottes Off-White sont un investissement mais celles-ci sont incroyables. Le cuir est de premi\u00e8re qualit\u00e9.", verified: true, daysAgo: 22, avatarIdx: 10 },
  ...
  // ==================== GUCCI LOAFER (18 reviews) ====================
  "gucci-chunky-platform-loafer-bee-black": {
    displayReviewCount: 18,
    reviews: [
      ...
      { customerName: "Marie Duval", rating: 5, comment: "Excellent quality! The leather is soft and comfortable from the first wear. The platform is stable and elegant.", commentFr: "Excellente qualit\u00e9! Le cuir est doux et confortable d\u00e8s le premier port. La plateforme est stable et \u00e9l\u00e9gante.", verified: true, daysAgo: 95, avatarIdx: 11 },
      ...
      { customerName: "Fatoumata Sy", rating: 5, comment: "The perfect loafers! Elegant with jeans or a dress. The platform is comfortable even after hours of walking.", commentFr: "Les mocassins parfaits! El\u00e9gants avec un jean ou une robe. La plateforme est confortable m\u00eame apr\u00e8s des heures de marche.", verified: true, daysAgo: 128, avatarIdx: 4 },
    ],
  },

  // ==================== NIKE CHUNKY TRAIL (newer - 15 reviews) ====================
  "nike-chunky-trail-sneaker-black-grey": {
    ...
    reviews: [
      ...
      { customerName: "Julie Rousseau", rating: 5, comment: "Love at first sight! Nike sneakers are always quality and these are no exception. Optimal comfort.", commentFr: "Achat coup de c\u0153ur! Les baskets Nike sont toujours de qualit\u00e9 et celles-ci ne font pas exception. Confort optimal.", verified: true, daysAgo: 30, avatarIdx: 10 },
      ...
      { customerName: "Thomas Lambert", rating: 5, comment: "These sneakers are a quality investment! The chunky trail design is trendy and comfort is amazing. I recommend!", commentFr: "Ces baskets sont un investissement de qualit\u00e9! Le design chunky trail est tendance et le confort est incroyable. Je recommande!", verified: true, daysAgo: 100, avatarIdx: 3 },
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let totalInserted = 0;
    const results: Array<{ slug: string; inserted: number; avgRating: number; displayReviewCount: number; error?: string }> = [];

    for (const product of allProducts) {
      const seed = PRODUCT_REVIEWS[product.slug];

      if (!seed) {
        results.push({ slug: product.slug, inserted: 0, avgRating: 0, displayReviewCount: 0, error: "No review template" });
        continue;
      }
      ...
      // Insert curated text reviews
      const reviewInserts = seed.reviews.map((r) => ({
      ...
      // Calculate avg from real reviews
      const totalRating = seed.reviews.reduce((sum, r) => sum + r.rating, 0);
      const avgRating = Math.round((totalRating / seed.reviews.length) * 10) / 10;

      // Update product: rating from real reviews, but inflated reviewCount for social proof
      await db
        .update(products)
        .set({
          rating: avgRating.toFixed(1),
          reviewCount: seed.displayReviewCount,
        })
        .where(eq(products.id, product.id));

      totalInserted += seed.reviews.length;
      results.push({
        slug: product.slug,
        inserted: seed.reviews.length,
        avgRating,
        displayReviewCount: seed.displayReviewCount,
      });
    }
