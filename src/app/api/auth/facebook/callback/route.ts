...
export async function GET(req: NextRequest) {
  ...
  const failLogin = role === "affiliate" ? `/${locale}/affiliate/login` : role === "vendor" ? `/${locale}/vendor/login` : `/${locale}/account/login`;

  if (!code) return NextResponse.redirect(new URL(`${failLogin}?error=oauth_failed`, req.nextUrl.origin));

  const appId = process.env.FACEBOOK_CLIENT_ID || process.env.FACEBOOK_APP_ID || process.env.NEXT_PUBLIC_FACEBOOK_APP_ID || "";
  const appSecret = process.env.FACEBOOK_CLIENT_SECRET || process.env.FACEBOOK_APP_SECRET || "";

  if (!appId || !appSecret) return NextResponse.redirect(new URL(`${failLogin}?error=oauth_not_configured`, req.nextUrl.origin));
  ...
  try {
    ...
    const userRes = await fetch(`https://graph.facebook.com/v19.0/me?fields=id,name,email,picture.type(large)&access_token=${tokens.access_token}`);
    ...
    if (role === "vendor") {
      ...
      if (!vend) {
        ...
        const [newVend] = await db.insert(vendors).values({
          email, passwordHash: dummyHash, storeName: storeName.slice(0, 100), storeSlug, contactName: name, status: "incomplete", mustChangePassword: false,
        }).returning();
        vend = newVend;
