...
<p>Nous acceptons le <strong>Bitcoin (BTC)</strong> et l'<strong>USDT (TRC20)</strong>. Pas de carte bancaire requise. Pas de relev\u00e9s bancaires. Abonnez-vous en moins de 60 secondes et commencez \u00e0 recevoir vos signaux dans l'heure.</p>
    ...
    const result = await db.update(products).set({
      ...
      updatedAt: new Date(),
    }).where(eq(products.slug, slug)).returning({ id: products.id, slug: products.slug });

    if (!result.length) {
      return NextResponse.json({ success: false, error: "Product not found for slug smz-ai-trading-bot-pro" }, { status: 404 });
    }

    return NextResponse.json({
      success: true,
      message: "SMZ EN+FR long descriptions updated",
      product: result[0],
      frChars: longDescFr.length,
      enChars: longDescEn.length,
    });
  } catch (e: any) {
