...
export default function AddressesPage() {
  ...
  return (
    <AccountShell title={isFr ? "Mes adresses" : "My Addresses"}>
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-[#CA3F2E] transition">
            <Plus className="w-4 h-4" /> {isFr ? "Ajouter une adresse" : "Add new address"}
          </button>
        )}
      </div>

      {showForm && (
        <div className="bg-white border-2 border-[#CA3F2E]/20 rounded-2xl p-6 mb-6 space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            <h2 className="font-bold text-gray-900">{isFr ? "Nouvelle adresse" : "New Address"}</h2>
            <button onClick={() => setShowForm(false)}><X className="w-4 h-4 text-gray-500" /></button>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <input placeholder={isFr ? "Nom complet" : "Full name"} value={form.fullName} onChange={(e) => setForm({...form, fullName: e.target.value})} className="col-span-2 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#CA3F2E]" />
            <input placeholder={isFr ? "T\u00e9l\u00e9phone" : "Phone"} value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#CA3F2E]" />
            <select value={form.label} onChange={(e) => setForm({...form, label: e.target.value})} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#CA3F2E]">
            ...
            <input placeholder={isFr ? "Rue" : "Street"} value={form.street} onChange={(e) => setForm({...form, street: e.target.value})} className="col-span-2 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#CA3F2E]" />
            <input placeholder={isFr ? "Ville" : "City"} value={form.city} onChange={(e) => setForm({...form, city: e.target.value})} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#CA3F2E]" />
            <input placeholder={isFr ? "\u00c9tat / R\u00e9gion" : "State / Region"} value={form.state} onChange={(e) => setForm({...form, state: e.target.value})} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#CA3F2E]" />
            <input placeholder={isFr ? "Pays" : "Country"} value={form.country} onChange={(e) => setForm({...form, country: e.target.value})} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#CA3F2E]" />
            <input placeholder={isFr ? "Code postal" : "Postal code"} value={form.postalCode} onChange={(e) => setForm({...form, postalCode: e.target.value})} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#CA3F2E]" />
          </div>
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm font-medium">{isFr ? "Adresse par d\u00e9faut" : "Default address"}</span>
          </label>
      ...
      {loading ? <Loader2 className="w-8 h-8 animate-spin mx-auto mt-12 text-[#CA3F2E]" /> : addresses.length === 0 ? (
        <div className="bg-white border border-gray-200/80 rounded-2xl p-12 text-center shadow-sm">
          ...
          <h2 className="text-lg font-bold mb-2 text-gray-900">{isFr ? "Aucune adresse" : "No addresses saved"}</h2>
        ...
        <div className="space-y-3">
          {addresses.map(a => (
            <div key={a.id} className="bg-white border border-gray-200/80 rounded-2xl p-4 flex items-start gap-4 shadow-sm hover:shadow-md transition">
