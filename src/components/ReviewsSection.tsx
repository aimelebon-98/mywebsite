...
const testimonials = [
  { name: "James Wilson",   avatar: "JW", rating: 5, comment: "Absolutely love these shoes! The comfort is incredible and they look even better in person. Best purchase I have made this year.", product: "Air Max Velocity",     color: "bg-blue-500"   },
  { name: "Sarah Chen",     avatar: "SC", rating: 5, comment: "The quality is outstanding. I have worn them daily for 3 months and they still look brand new. Highly recommend SoleVault!",    product: "Cloud Walker",         color: "bg-pink-500"   },
  ...
  { name: "Emily Parker",   avatar: "EP", rating: 5, comment: "I am obsessed with these! Got so many compliments. The WhatsApp ordering was super easy too. Will definitely buy again.",        product: "Neon Pulse",           color: "bg-purple-500" },
      ...
      <div className="relative mb-4">
        <div className="flex gap-4 animate-marquee">
          {[...testimonials, ...testimonials].map((review, i) => (
            <ReviewCard key={`r1-${i}`} review={review} boughtLabel={t("reviewsBought")} verifiedLabel={t("reviewsVerified")} />
          ))}
        </div>
      </div>
      ...
      <div className="relative">
        <div className="flex gap-4 animate-marquee" style={{ animationDirection: "reverse", animationDuration: "35s" }}>
          {[...testimonials.slice(4), ...testimonials.slice(0, 4), ...testimonials.slice(4), ...testimonials.slice(0, 4)].map((review, i) => (
            <ReviewCard key={`r2-${i}`} review={review} boughtLabel={t("reviewsBought")} verifiedLabel={t("reviewsVerified")} />
          ))}
        </div>
      </div>
    </section>
    ...
    <div className="flex-shrink-0 w-[340px] bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400">{boughtLabel} {review.product}</p>
        </div>
