...
interface Props { product: { id: string; name: string; slug: string; subscriptionConfig?: string | null }; locale?: string; }
    ...
    <>
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-center gap-4 text-xs text-gray-500 dark:text-gray-400">
          <span className="flex items-center gap-1"><DollarSign className="w-3.5 h-3.5 text-emerald-500" />{isFr ? "1, 3, 5 ou 12 mois" : "1, 3, 5 or 12 mos"}</span>
          <span className="text-gray-300">|</span>
          <span className="flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />{isFr ? "Activation auto" : "Auto-activation"}</span>
        </div>
      </div>
      <SubscriptionCheckoutModal product={product} isOpen={open} onClose={() => setOpen(false)} locale={locale} />
    </>
  );
}